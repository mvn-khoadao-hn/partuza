...
//$this->cache = Cache::createCache(Config::get('data_cache'), 'PartuzaDbFetcher');
  ...
  public function getCoin($userId, $appId, $fields, $options, $token) {
    //$first = $options->getStartIndex();
    //$max = $options->getCount();
    $this->checkDb();
    $ret = array();

    $query = "select * from person_coins where person_id = $userId and app_id = $appId";
    $res = mysqli_query($this->db, $query);
    if ($res) {
      while ($row = @mysqli_fetch_array($res, MYSQLI_ASSOC)) {
        $payment_id = $row['person_id'];
        $payment = new Coin($payment_id, $row['app_id']);
        $payment->setCoin($row['coin']);
        $ret[$payment_id] = $payment;
      }
    }
  ...
  public function checkCoin($userId, $appId, $coin) {
    $this->checkDb();
    $query = "select * from person_coins where person_id = '$userId' and app_id = '$appId'";
    $res = mysqli_query($this->db, $query);
    if ($res && (@mysqli_num_rows($res) > 0)) {
	...
	if($coin < 0) {
	  //throw new Exception("Coin of people not enough (can't below 0)!");
	  return false;
 	}		
    }
    else {
  ...
  public function updateCoin($userId, $appId, $coin) {
    $this->checkDb();
    $query = "select * from person_coins where person_id = '$userId' and app_id = '$appId'";
    $res = mysqli_query($this->db, $query);
    if ($res && (@mysqli_num_rows($res) > 0)) {
	...
	if($coin < 0) {
	    //throw new Exception("Coin of people can't below 0!");
            return false;
 	}		
 ...
 function insertPayment($user_id, $callbackUrl, $finishPageUrl, $message, $paymentItems, $now, $coin){
    $this->checkDb();
    $query = "insert into payments (callbackUrl, finishPageUrl, message, items, ordered_date, coin)" . " values ('$callbackUrl', '$finishPageUrl', '$message', '$paymentItems','$now',$coin)";
    mysqli_query($this->db, $query);
    $id = mysqli_insert_id($this->db);
    if (!$id) {
      throw new SocialSpiException("Insert payments failed.", ResponseError::$INTERNAL_ERROR);
    } else {
      return $id;
    }
 }
 ...
 function insertPersonPayment($payment_id, $app_id, $platform_user_id){
    $this->checkDb();
    $query = "select * from person_payments where payment_id = $payment_id and person_id = $platform_user_id";
    $res = mysqli_query($this->db, $query);
    if ($res && (@mysqli_num_rows($res) > 0)) {
	throw new SocialSpiException("Insert person payments failed. Person payment was already exist.", ResponseError::$INTERNAL_ERROR);
    }
	...
	$diff = (strtotime($now) - strtotime($ordered_date))/ 60;
	if($diff > 30) {
	   $status = 4;	//expired
	}
	else {
	   $status = 2;
	   $query = "insert into person_payments (person_id, payment_id, app_id)" . " values ($platform_user_id, $payment_id, $app_id)";
	   mysqli_query($this->db, $query);
	   mysqli_insert_id($this->db);
	   $query = "update payments set executed_date = '$now' where id = $payment_id";
	   if (! mysqli_query($this->db, $query)) {
	       throw new SocialSpiException("Update payments failed.", ResponseError::$INTERNAL_ERROR);
	   }
	}
    	$result = $callbackURL . "?paymentId=" . $payment_id . "&status=" . $status . "&opensocial_app_id=" . $app_id . "&opensocial_viewer_id=" .  $platform_user_id . "&opensocial_owner_id=" .  $platform_user_id . "&orderedTime=" . $now;
    	return $result;
    }
    else {
	throw new SocialSpiException("Insert person payments failed. Payment was not exist!", ResponseError::$INTERNAL_ERROR);
    }
 }

  public function getPayment($userId, $appId, $paymentId, $token) {
    $this->checkDb();
    $query = "select * from person_payments where payment_id = $paymentId and person_id = $userId";
    $res = mysqli_query($this->db, $query);
    if ($res && (@mysqli_num_rows($res) > 0)) {      
        $payment = null;
	...
	if ($res && (@mysqli_num_rows($res) > 0)) {
	   while ($row = @mysqli_fetch_array($res, MYSQLI_ASSOC)) {
		$payment_id = $row['id'];
		$payment = new Payment($payment_id, $row['message'], '1', $row['items'], $row['finishPageUrl']);
	   }
	}
	return $payment;
    }
    else {
	throw new SocialSpiException("Payments wasn't found.", ResponseError::$INTERNAL_ERROR);
    }
  }

  public function getIgnoreList($userId, $ownerId) {
    $this->checkDb();
    $query = "select * from ignorelists where person_id = $ownerId and target_id = $userId";
    $res = mysqli_query($this->db, $query);
    if ($res && (@mysqli_num_rows($res) > 0)) {
        $result = array();
	while ($row = @mysqli_fetch_array($res, MYSQLI_ASSOC)) {
	   $ignorelist = new IgnoreList($ownerId, $row["id"]);
	   array_push($result, $ignorelist);
	}
	return $result;
    }
    else {
