...
//$this->cache = Cache::createCache(Config::get('data_cache'), 'PartuzaDbFetcher');
 ...
 function insertPayment($user_id, $callbackUrl, $finishPageUrl, $message, $paymentItems,$now){
    $this->checkDb();
    $query = "insert into payments (callbackUrl, finishPageUrl, message, items, ordered_date)" . " values ('$callbackUrl', '$finishPageUrl', '$message', '$paymentItems','$now')";
    mysqli_query($this->db, $query);
    $id = mysqli_insert_id($this->db);
    if (!$id) {
