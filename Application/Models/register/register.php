...
class registerModel extends Model {
  ...
  function registerAPI($email, $passwd, $first_name, $last_name, $gender) {
    global $db;
    $email = $db->addslashes(trim($email));
    $passwd = $db->addslashes(trim($passwd));
    $first_name = $db->addslashes(ucwords(trim($first_name)));
    ...
    // ok email is free, create the persons record
    $res = $db->query("insert into persons (email, password, first_name, last_name, gender) values ('$email', PASSWORD('$passwd'), '$first_name', '$last_name', '$gender')");
    $id = $db->insert_id($res);
    // and return the newly created persons record id
    return $id;
  }
